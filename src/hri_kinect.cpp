...
void hri_draw_kinect_human_arms(HRI_AGENTS* agents)
{
    ...
    {
        p3d_rob* rob = agents->humans[i]->robotPt;

        string str( rob->name );

        //cout << "hri_draw_kinect_human_arms : " << str << endl;

        if( str.find("HERAKLES_HUMAN") == string::npos )
            continue;
        ...
        // Bras Droit
        jnt = p3d_get_robot_jnt_by_name( rob, (char*) "rShoulderX" );
        if(!jnt)
        {
            error = true;
            continue;
        }
        p3d_jnt_get_cur_vect_point( jnt , pos1 );

        jnt = p3d_get_robot_jnt_by_name( rob, (char*) "rElbowZ" );
        if(!jnt)
        {
            error = true;
            continue;
        }
        p3d_jnt_get_cur_vect_point( jnt , pos2 );

        if ( p3d_vectDistance( pos1, pos2) > 0.10 )
        {
            ...
            g3d_draw_cylinder( pos3, pos2, 0.04, 20 );
        }

        // Bras Gauche
        jnt = p3d_get_robot_jnt_by_name( rob, (char*) "lShoulderX" );
        if(!jnt)
        {
            error = true;
            continue;
        ...
        jnt = p3d_get_robot_jnt_by_name( rob, (char*) "lElbowZ" );
        if(!jnt)
        {
            error = true;
            continue;
